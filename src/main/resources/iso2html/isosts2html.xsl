...
<xsl:stylesheet version="2.0"
  ...
  exclude-result-prefixes="xs xlink mml tbx xhtml isosts loc mle tmp">
	...
	<!-- ============================================================= -->
	<xsl:template match="comment | source | publisher-name | publisher-loc | article-title | source | year | volume | edition | fpage | lpage | size | chapter-title | issue | supplement | series">
		<xsl:apply-templates/>
	</xsl:template>
	...
	<xsl:template name="write-name" match="name">
		...
		<xsl:apply-templates mode="inline-name"/>
	...
	<xsl:template match="given-names" mode="inline-name">
		<xsl:apply-templates/>
		<!-- ISO doesn't use @name-style='eastern'-->
		<xsl:if test="not((../following-sibling::name) | (../following-sibling::etal))">
			<xsl:text>,</xsl:text>
		</xsl:if>
	</xsl:template>
	...
	<xsl:template match="contrib/name/surname" mode="inline-name">
		<xsl:apply-templates/>
		<xsl:if test="../given-names | ../suffix">
			<!-- ISO doesn't use @name-style='eastern'-->
		...
		<div class="sts-section sts-ref-list">
			...
			<xsl:for-each select="title">
				<xsl:call-template name="sec-title">
					<xsl:with-param name="level" select="count(ancestor::ref-list)+1"/>
				</xsl:call-template>
			</xsl:for-each>
	...
	<xsl:template match="disp-formula">
		<div class="sts-{local-name()}-panel">
			...
			<xsl:call-template name="named-anchor"/>      
			<xsl:apply-templates/>
			<div class="sts-{local-name()}-label">
				<xsl:value-of select="label"/>
			</div>
		</div>
	</xsl:template>
	...
	<xsl:template match="graphic | inline-graphic">
		...
		<img alt="{$alt-text}" src="{@xlink:href}">
	...
	<xsl:template match="std[not(title)]" priority="1">
		<xsl:choose>
			<xsl:when test="@std-id">
				<a class="sts-std-ref" href="#{@std-id}">
					<xsl:apply-templates/>
				</a>
			</xsl:when>
			<xsl:otherwise>
	...
	<xsl:template match="non-normative-example/label">
		<p>
			<span class="sts-non-normative-example-label">
				<xsl:apply-templates/>
			</span>
		</p>
	</xsl:template>
	...
	<xsl:template match="non-normative-note/label[not(following-sibling::p)]">
		<p>
			<span class="sts-non-normative-note-label">
				<xsl:apply-templates/>
			</span>
		</p>
	</xsl:template>
	...
	<xsl:template match="tbx:definition">
		<div class="sts-tbx-def">
			<xsl:if test="following-sibling::tbx:subjectField!=''">
				<xsl:text>&lt;</xsl:text>
				<xsl:value-of select="following-sibling::tbx:subjectField"/>
				<xsl:text>&gt;</xsl:text>
				<xsl:text> </xsl:text>
			</xsl:if>
			<xsl:apply-templates/>
	...
	<xsl:template match="tbx:example">
		<div class="sts-tbx-example">
          ...
          &#xA0;<xsl:number/>
			...
			<div class="sts-tbx-example-content">
				<xsl:apply-templates/>
			</div>
		</div>
	</xsl:template>
		...
		<xsl:variable name="href">
			<xsl:choose>
				<xsl:when test="@target">
					<xsl:value-of select="concat($urn, ':', replace(@target,'_',':'))"/>
				</xsl:when>
				<xsl:otherwise>
			...
			<a href="#{$href}">
				<xsl:choose>
					<xsl:when test="normalize-space(.) = ''">
						...
						<xsl:apply-templates select="//tbx:termEntry[@id=$target]/tbx:langSet/tbx:tig[1]/tbx:term[1]/(*|text())"/>
					</xsl:when>
					<xsl:otherwise>            
						<xsl:apply-templates/>
					</xsl:otherwise>
	...
	<xsl:template match="mml:*" mode="mml-copy">
		<xsl:element name="{local-name(.)}">
			<xsl:apply-templates select="@*" mode="mml-copy"/>
			<xsl:apply-templates mode="mml-copy"/>
		</xsl:element>
	</xsl:template>

	<xsl:template match="@*" mode="mml-copy">
		<xsl:copy-of select="."/>
	</xsl:template>
	...
	<xsl:template match="collab">
		<xsl:apply-templates/>
		<!-- ISO doesn't use @name-style='eastern'-->
		<xsl:if test="not(../following-sibling::name)">
			<xsl:text>,</xsl:text>
		</xsl:if>
	</xsl:template>
