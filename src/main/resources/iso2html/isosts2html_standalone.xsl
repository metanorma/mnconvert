...
<!--  $LastChangedRevision: 49876 $                                -->
<!--  $LastChangedDate: 2013-12-05 23:59:52 +0100 (Thu, 05 Dec 2013) $                                           -->
<!--                                                               -->
<!-- ============================================================= -->
  ...
  <xsl:template name="assign-src">
    <xsl:if test="@xlink:href">
      <xsl:attribute name="src">
        <xsl:choose>
          <xsl:when test="$img-src='o-drive'">
            <xsl:choose>
              <xsl:when test="(self::graphic or self::inline-graphic) and processing-instruction(isoimg-id)">
                <xsl:variable name="isoimg-id" select="processing-instruction(isoimg-id)"/>
                <xsl:value-of select="concat('file://csgva.iso.ch/shares/StandData/Draw/Png/', substring($isoimg-id, 1, 4), '/', functx:substring-before-last($isoimg-id, '.'), '.PNG')"/>
              </xsl:when>
              <xsl:otherwise>
      ...
      <xsl:if test="$img-src='o-drive'">
        <xsl:attribute name="style">max-width: 50%; max-height:50%;</xsl:attribute>
      </xsl:if>
    </xsl:if>
  </xsl:template>
      ...
      <xsl:attribute name="src">
        <xsl:choose>
          <xsl:when test="$img-src='o-drive'">            
              ...
              <xsl:value-of
                select="concat('file://csgva.iso.ch/shares/StandData/Draw/Graphics/',
